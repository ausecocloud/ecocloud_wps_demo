...
class ExploratoryDataHistogram(Process):
    def __init__(self):
        inputs = [
            ComplexInput(
        ...
        super(ExploratoryDataHistogram, self).__init__(
            self._handler,
            identifier='exploratory_data_histogram',
            title='Exploratory data: Histogram plot',
            abstract='Generates histogram plots from one or more variables in the provided CSV dataset',
            version='1',
            metadata=[],
            inputs=inputs,
            outputs=outputs,
    ...
    def _handler(self, request, response):
        ...
        for idx, var in enumerate(variables):
            ...
            axes.set_title(var)
            axes.hist(column_data, bins='auto')
